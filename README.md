# React + TypeScript + Vite

```
npm i
npm run dev
npm run deploy
```

## TODO list

### general

- [ ] Иконки !SVG (есть в фигме ЧОП)
- [ ] Изображения !в правильном формате

  ### style

- [x] Поправить лого в секции hero в home.tsx
- [x] Добавить круги на фон секции advantages в home.tsx
- [ ] Адаптив для планшетов
- [x] Дизайн PageTitile comonent
- [x] Пагинация к слайдерам
- [x] Страница О компании
- [x] Страница Услуги
- [x] Страница Услуги:услуга
- [x] Страница Объекты
- [x] Страница Контакты
- [x] Страница 404
- [ ] Страница Калькулятор
- [ ] Поправить все телефоны на <phoneComponent>
- [x] ссылки в footer
- [x] m: paddimg карт в мобилке
- [x] m: padding-top на странице О комп от фото
- [x] m: padding-x Home-page О комп
- [x] Добавить кнопку "перезвоните"
- [x] m: стилизовать лого (разделить на 2, текст показывать при открытии хамб меню)
- [x] hover у кнопок исправить
- [x] m: страница конкретной услуги Safari длинный блок баг
- [x] Components без marin!
- [x] Зум формы Safari
- [ ] ProposalComonent заменеить кнопку "Наши услуги" на "перезвонить"

  ### functional

- [ ] Админка
- [ ] !!! Хранение данных
- [x] m: Fix Активной ссылки в бурге меню
- [x] Скачивание файлов в лицензиях
- [x] Валидация feedbackform
- [ ] Маска телефонп
- [x] Вынести навигацию в компонент
- [x] Error Boundary
- [x] Иконки в теге img перевести в svg (для перекраски)
- [x] !!! доступ к услуге по прямой ссылке
- [ ] city React router

```Синхронизация в URL: Если вам нужно, чтобы для каждого города была своя уникальная ссылка (например, для SEO: /moscow/contacts), то вместо Context лучше использовать React Router. Параметр города будет храниться в URL (/:city/contacts), а хук useParams() заменит useCity().

```

- [ ] \*В синем хэдере сверху нужно справа заменить текст на выбор города, в зависимости от города будет меняться почта и телефон там же.
- [ ] \*В контактах нужен список по городам номеров и прочего(несколько блоков которые с иконками). С картой так же несколько по городам. Нужен наверно общий список для перехода по ссылкам или менять карту от выбора города.
- [ ] \*В футере так же контакты меняются от города

### strapi \ api \ cms \

- [ ] вынести интерфейсы
- [ ] Регулярки
- [ ] Контакты в Single Types
- [ ] постфиксы данных ex: m^2, человеки итд

## App structure

```bash
src/
├───assets    # Статические файлы (изображения, шрифты)
│   └───ServicesData
├───components    # Глобальные повторяемые компоненты (Button, Header, Footer)
│   └───Header
├───features     # Логика конкретной функциональности
│   └───FeedbackForm    # Главный компонент формы обратной связи
├───layouts      # Обертки для страниц (Header, Footer, Sidebar)
└───pages   # Страницы приложения
    ├───About
    │   └───components
    ├───Home
    │   └───components
    ├───NotFound
    ├───Objects
    │   └───components
    ├───ServiceDetail
    └───Services
        └───components
```
